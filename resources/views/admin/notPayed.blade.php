...
<script>
  ...
  $(".dt-buttons .buttons-excel").hide();
            if($('#delivery-filter').val()){
                $(".dt-buttons .buttons-excel").show();
            }
            $('.row-checkbox').on('click', function() {
                if ($(".row-checkbox:checked").length > 0) {
                    $('#markAsPayed').show();
                    $('#markAsPayed').addClass("return-button")
                } else {
                    $('#markAsPayed').hide();
                    $('#markAsPayed').removeClass("return-button");
                }
    ...
    $("button#markAsPayed").click(function() {
        let ids = [];
...
if (e.status == "Livré" && e.delivery_id == $('#delivery-filter').val()) {
 ...
 const totalPrice = data.reduce((sum, parcel) => sum + parseFloat(parcel.price), 0);
    ...
    doc.autoTable({
            ...
            startX:100,
            body:[ ["totale",totalPrice],
              ["to be paid",totalPrice-com],
              ["Commission deliveryman",com]], 
            styles: {
                fontSize: 10
            },
