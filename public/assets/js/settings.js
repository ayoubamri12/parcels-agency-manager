...
$(document).ready(function () {
    var table1 = $('#table').DataTable({
        ...
        },
        {
            ...
            render: (data, type, row) => {
              let  deliveryman_number=[]
                row.deliveriyman_cities.map(e=>{
                    if(!deliveryman_number.includes(e.delivery_id)){
                        deliveryman_number.push(e.delivery_id)
                    } 
                })
                return `${deliveryman_number.length
                    }`
    ...
    $('#merge').on('submit', function (e) {
        ...
        let local = $('input[name="cps[]"]:checked');

            if (!local.length>0) {
                isValid = false;
                $('.checkbox-list1').parents(".dropdown-assign").after(
    ...
    $('#LocationForm').on('submit', function (e) {
        ...
        let selectedCompany = $('input[name="local_reg"]:checked');

        if (!selectedCompany.length) {
            isValid = false;
            
            $('input[name="local_reg"]:last').parent("div").after(
                '<div class="error d-block" style="color: red;">Location is required.</div>');
        }
        if (isValid) {
            // Submit the form if everything is valid
            this.submit();
            ...
            },
            {
                ...
                render: function (data, type, row) {
                    ...
                    <span id='put' style='display:none'>
                        <input type="text" id='passw${row.id}' class="form-control password-input" style='border-radius:50px;' name="updatePassword" placeholder="Enter new password">
                        </span>
